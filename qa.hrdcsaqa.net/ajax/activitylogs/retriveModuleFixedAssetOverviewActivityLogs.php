...
if(isset($_SESSION['accesses']->_login['id']) && (count($_SESSION['accesses']->_login) >= 1) && isset($_GET['_module']) && ($_GET['_module'] >= 0) && (isset($_GET['_employee']) || ($_GET['_employee'] >= 0))  )
{
	...
	$_ORG_NAME = $_ORG_LOGO = "";
	$_MODULE_NAME[1] = "Fixed Asset Register";
	...
	//$_brandArr = genericItemsArr("lists"," AND `category`=49 ");//print_r($_cl
	$_brandArr = genericItemsArr("fixed_asset_brands"," AND `company`=".$_company." ");//print_r($_classificationArr);
	...
	if(isset($_GET['_employee']) && ($_GET['_employee'] >= 1) && ($_GET['_employee'] != "null"))
	{
		$_SQL_LIMIT .= " AND `A`.`asset_location` IN (".$_GET['_employee'].") ";
		
	}
	
	if(isset($_GET['_classification']) && ($_GET['_classification'] >= 1) && ($_GET['_classification'] != "null"))
	{
		$_SQL_LIMIT .= " AND `A`.`asset_classification` IN (".$_GET['_classification'].") ";
		
	}
	
	if(isset($_GET['_brand']) && ($_GET['_brand'] >= 1) && ($_GET['_brand'] != "null"))
	{
		$_SQL_LIMIT .= " AND `A`.`asset_brand` IN (".$_GET['_brand'].") ";
		
	}
	
	$_EXUECTUTE = true;
	
	$fname = "Fixed Asset Register Overview ";
	...
	$_body .= "
			...
			<td style='background:#e3e3e3;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='center'>Brand</td>
	...
	//$_SQL_INVENTORY_SELECT .= ",`A`.`asset_barcode` AS `_barcode`";
	$_SQL_INVENTORY_SELECT .= ",`A`.`asset_serial_no` AS `_barcode`";
	$_SQL_INVENTORY_SELECT .= ",`A`.`asset_brand` AS `_brand`";
	$_SQL_INVENTORY_SELECT .= ",`A`.`asset_serial_no` AS `_serial_no`";
	$_SQL_INVENTORY_SELECT .= ",`A`.`asset_name` AS `_name`";
	$_SQL_INVENTORY_SELECT .= ",'1' AS `_qty`";$_SQL_INVENTORY_ORDER_BY = "ORDER BY `A`.`asset_location` ASC,`A`.`asset_classification` DESC";
	$_SQL_INVENTORY_SELECT .= ",`A`.`asset_purchase_price` AS `_cost`";
	$_SQL_INVENTORY_SELECT .= ",`A`.`asset_location` AS `_bin`";
	$_SQL_INVENTORY_SELECT .= ",`A`.`asset_classification` AS `_category`";
	$_SQL_INVENTORY_SELECT .= ",`A`.`asset_brand` AS `_brand`";
	$_SQL_INVENTORY_SELECT .= ",`A`.`asset_description` AS `_description`";
	...
	$_EQUIPMENT = $_MACHINERY = $_VEHICLE = $_COMPUTER = 0;
	...
	if(($rs) && ($rs->numRows() >= 1))
	{
		...
		while(!$rs->EOF)
		{
			...
			$bin = isset($_locationArr[$rs->fields['_bin']]) ? $_locationArr[$rs->fields['_bin']] : "";
			$brand = isset($_brandArr[$rs->fields['_brand']]) ? $_brandArr[$rs->fields['_brand']] : "";
			...
			//echo $category."<br/>";
			if($category == "EQUIPMENT"){$_EQUIPMENT = $_EQUIPMENT +  $rs->fields['_cost'];}
			else if($category == "MACHINERY"){$_MACHINERY = $_MACHINERY +  $rs->fields['_cost'];}
			else if($category == "COMPUTERS"){$_COMPUTER = $_COMPUTER +  $rs->fields['_cost'];}
			else if($category == "VEHICLE"){$_VEHICLE = $_VEHICLE +  $rs->fields['_cost'];}
			
			$_TOTAL = $_TOTAL + $rs->fields['_cost'];
			...
			$_body .= "
					...
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='center'><b></b>".$brand."</td>
			...
			$_DATA_ARR[] = array(substr($itemName,0,18),$rs->fields['_barcode'],$category,$bin,substr($rs->fields['_description'],0,10),$rs->fields['_cost'],$rs->fields['_qty'],0,(isset($rs->fields['_qty']) && ($rs->fields['_qty'] != 0)?"R ".number_format(round($rs->fields['_qty']*$rs->fields['_cost'],2),2,"."," ")."":""),$brand,"-----");
	...
	if(!(isset($_GET['_disp']) && ($_GET['_disp'] >= 1)))
	{
		...
		$_GET['is_pdf']=1;//echo $fname." - ".$_GET['_module'];
	...
	if((isset($_GET['_isemail']) && ($_GET['_isemail'] >= 1)) || (isset($_GET['is_excel']) && ($_GET['is_excel'] >= 1)) || (isset($_GET['is_pdf']) && ($_GET['is_pdf'] >= 1)))
	{
		...
		// _____________________________________________________ Page # 1 - Cover Page __________________________________________________
				...
				$pdf->SetFont('arial','',11);$pdf->SetTextColor(135);$pdf->SetXY($w+3.5+($_P_WIDTH/2.5),$h+17.55);$pdf->Cell($_P_WIDTH/2.61,6,"( Total Fixed Asset Value ) ",0,0,"C",false);
				
				$h = $_START_TOP;$w = $_START_LEFT;
				
				$pdf->SetTextColor(0);$pdf->SetFont('arial','',34);
				
				$pdf->SetXY(70-2,$h+1.85);$pdf->MultiCell($_HEIGHT,15,"Fixed Asset Register \nOverview",0,"L");
				...
				//$pdf->SetTextColor(100);$pdf->SetFont('arial','',18);$pdf->Text($xPos,$yPos,"Fixed Asset Register Overview");$yPos+=8;
				...
				$pdf->SetFont('arial','',10);
					...
					$pdf->SetXY($w+50,$h);$pdf->MultiCell(40,6,"Serial No.",1,'C',true);
					$pdf->SetXY($w+50+40,$h);$pdf->MultiCell(30,6,"Classification",1,'L',true);
					$pdf->SetXY($w+50+40+30,$h);$pdf->MultiCell(30+30,6,"Wareh. Bin Loc.",1,'C',true);
					//$pdf->SetXY($w+50+40+25+35,$h);$pdf->MultiCell(30,6,"Description",1,'L',true);
					$pdf->SetXY($w+50+40+15+25+20+30,$h);$pdf->MultiCell(25,6,"Brand",1,'R',true);
				...
				$_INDEX = 0;$_CUT_OFF = 20;//print_r($_DATA_ARR);
				foreach($_DATA_ARR as $k=>$v)
				{
					...
					$pdf->SetTextColor(0);$pdf->SetFillColor(255);
					    ...
					    $pdf->SetXY($w+50+40+30,$h);$pdf->MultiCell(30+30,5,$v[3],1,'L');//$pdf->SetXY($w+50+40+25+35,$h);$pdf->MultiCell(30,5,substr($v[4],0,15),1,'L');
					    $pdf->SetXY($w+50+40+15+25+20+30,$h);$pdf->MultiCell(30,5,$v[9],1,'L');
					    ...
					    $pdf->SetXY($w+50+40+15+20+20+30+30+25,$h);$pdf->MultiCell(20,5,"1",1,'C');$pdf->SetXY($w+50+40+15+20+20+30+30+30+15,$h);$pdf->MultiCell(30,5,"R ".$v[5],1,'R',true);
				...
				$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Inventory (Stock)",0,"L");$pdf->SetXY($_START_LEFT+105,$h);$pdf->Cell(30,6,"R0.00",0,"R","R");$pdf->SetXY($_START_LEFT+110+30,$h);$pdf->Cell(30,6,"R0.00",0,"R","R");$h+=6;
				$pdf->SetFont('arial','B',10);$pdf->SetTextColor(25);
				$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Total current assets","TB","L");$pdf->SetXY($_START_LEFT+101,$h);$pdf->Cell(34,6,"R0.00","TB","R","R");$pdf->SetXY($_START_LEFT+106+30,$h);$pdf->Cell(34,6,"R0.00	","TB","R","R");$h+=6;
				...
				$pdf->SetFont('arial','',10);$pdf->SetTextColor(105);echo $_VEHICLE;
				...
				$pdf->SetTextColor(245,0,0);$pdf->SetFont('arial','B',10);$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Furniture and Fixtures",0,"L");$pdf->SetXY($_START_LEFT+105,$h);$pdf->Cell(30,6,"R0.00",0,"R","R");$pdf->SetXY($_START_LEFT+110+30,$h);$pdf->Cell(30,6,"R0.00",0,"R","R");$h+=6;
				$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Vehicles",0,"L");$pdf->SetXY($_START_LEFT+105,$h);$pdf->Cell(30,5,"R 0.00",0,"R","R");$pdf->SetXY($_START_LEFT+110+30,$h);$pdf->Cell(30,6,"R ".number_format(round($_VEHICLE,2),2,"."," "),0,"R","R");$h+=6;
				$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Machinery and equipment",0,"L");$pdf->SetXY($_START_LEFT+105,$h);$pdf->Cell(30,6,"R 0.00",0,"R","R");$pdf->SetXY($_START_LEFT+110+30,$h);$pdf->Cell(30,6,"R ".number_format(round(($_EQUIPMENT + $_MACHINERY),2),2,"."," "),0,"R","R");$h+=6;
				$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Computer hardware, tablets, software",0,"L");$pdf->SetXY($_START_LEFT+105,$h);$pdf->Cell(30,6,"R0.00",0,"R","R");$pdf->SetXY($_START_LEFT+110+30,$h);$pdf->Cell(30,6,"R ".number_format(round($_COMPUTER,2),2,"."," "),0,"R","R");$h+=6;
				$pdf->SetFont('arial','',10);$pdf->SetTextColor(105);$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Research and development",0,"L");$pdf->SetXY($_START_LEFT+105,$h);$pdf->Cell(30,6,"R0.00",0,"R","R");$pdf->SetXY($_START_LEFT+110+30,$h);$pdf->Cell(30,6,"R0.00",0,"R","R");$h+=6;
				$pdf->SetFont('arial','B',10);$pdf->SetTextColor(25);
				...
				$pdf->SetTitle("Fixed Asset Register Overview - ".$_GET['_module']." - ".date("Y-F-d")."");
