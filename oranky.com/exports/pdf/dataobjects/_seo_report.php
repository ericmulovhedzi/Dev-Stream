...
if(true)
{
	...
	require(ROOTPATH."inc/pdf/fpdf186/fpdf.php");
	...
	//$pdf=new PDF_Ellipse('P','mm',array($_WIDTH,$_HEIGHT));$pdf->Open();
	$pdf = new FPDF('P','mm',array($_WIDTH,$_HEIGHT));
	
	
	$pdf->SetTextColor(0);$pdf->SetFont('arial','',11);$pdf->SetDrawColor(0);
	...
	if(true)
	{
		...
		$pdf->Rect(0,0,$_WIDTH,$_HEIGHT/2,0,'F');
	       ...
	       if($_TOTAL_AGGREGATE>0){$_AGGREGATE_RATE = round(((100*$_TOTAL_SUCCESS)/$_TOTAL_AGGREGATE),2);}
		...
		//$pdf->Circle($_WIDTH/2,$h+50,27,'');
		$pdf->Text(($_WIDTH/2)-23.0,$h+56.75,number_format($_AGGREGATE_RATE,2));
		...
		$pdf->SetFillColor($C_A['r'][0],$C_A['g'][0],$C_A['b'][0]);$pdf->SetTextColor(255);$pdf->SetFont('arial','B',11);$pdf->Rect($w+126,$h+2.5,8,5,1,'F');$pdf->Text($w+126+1.75,$h+2.5+3.75,$_TOTAL_DANGER);
		...
		$pdf->SetFillColor($C_A['r'][1],$C_A['g'][1],$C_A['b'][1]);$pdf->SetTextColor(255);$pdf->SetFont('arial','B',11);$pdf->Rect($w+126,$h+2.5,8,5,1,'F');$pdf->Text($w+126+1.75,$h+2.5+3.75,$_TOTAL_WARNING);
		...
		$pdf->SetFillColor($C_A['r'][2],$C_A['g'][2],$C_A['b'][2]);$pdf->SetTextColor(255);$pdf->SetFont('arial','B',11);$pdf->Rect($w+126,$h+2.5,8,5,1,'F');$pdf->Text($w+126+1.75,$h+2.5+3.75,$_TOTAL_SUCCESS);
		...
		$pdf->SetFillColor($C_A['r'][3],$C_A['g'][3],$C_A['b'][3]);$pdf->SetTextColor(255);$pdf->SetFont('arial','B',11);$pdf->Rect($w+126,$h+2.5,8,5,1,'F');$pdf->Text($w+126+1.75,$h+2.5+3.75,$_TOTAL_INFORMATIONAL);
		...
		$pdf->SetFillColor(81,86,89);$pdf->Rect(20-5,$h-25,31.5,30,3,'F');
		...
		$pdf->SetTextColor(255);$pdf->Rect($w+1+15,$h-16,75,8,1,'F');$pdf->Text($w+4+15,$h+9.5-20,$_REQUEST_URL);
		...
		$pdf->SetAlpha(0.15);$pdf->Image(ROOTPATH."images/icons/question-mark-black.png",$w+71+36.5+15.5,$h+1.75,6,0,'',''); 
		$h+=8;
		$pdf->SetXY($w,$h);$pdf->MultiCell(71,8,"5. Leadership ",0,'R');
		$pdf->SetXY($w+71+0.5,$h);$pdf->MultiCell($_P_WIDTH/8+15-0.15,8,"","B",'L');$pdf->SetXY($w+71+0.5*2+($_P_WIDTH/8+15),$h);$pdf->MultiCell($_P_WIDTH/8+15-0.15,8,"","B",'L');$pdf->SetXY($w+71+0.5*3+($_P_WIDTH/8+15)*2,$h);$pdf->MultiCell($_P_WIDTH/8+15-0.15,8,"","B",'L');
		$pdf->Image(ROOTPATH."images/icons/tick-black.png",$w+71+0.5+15.5,$h+2.75,4,0,'','');
		$pdf->SetAlpha(0.15);$pdf->Image(ROOTPATH."images/icons/question-mark-black.png",$w+71+36.5+15.5,$h+1.75,6,0,'','');$pdf->Image(ROOTPATH."images/icons/question-mark-black.png",$w+71+73+15.5,$h+1.75,6,0,'',''); 
		$h+=8;
		...
		$pdf->SetXY($w+71+0.5,$h);$pdf->MultiCell($_P_WIDTH/8+15-0.15,8,"","B",'L');$pdf->SetXY($w+71+0.5*2+($_P_WIDTH/8+15),$h);$pdf->MultiCell($_P_WIDTH/8+15-0.15,8,"","B",'L');$pdf->SetXY($w+71+0.5*3+($_P_WIDTH/8+15)*2,$h);$pdf->MultiCell($_P_WIDTH/8+15-0.15,8,"","B",'L');
		$pdf->SetAlpha(0.15);$pdf->Image(ROOTPATH."images/icons/question-mark-black.png",$w+71+15.5,$h+1.75,6,0,'',''); 
		$pdf->SetAlpha(0.15);$pdf->Image(ROOTPATH."images/icons/question-mark-black.png",$w+71+36.5+15.5,$h+1.75,6,0,'','');$pdf->Image(ROOTPATH."images/icons/question-mark-black.png",$w+71+73+15.5,$h+1.75,6,0,'',''); 
		$h+=8;
		...
		*/
		...
		$pdf->SetXY($w,$h);$pdf->Cell(13,6.2,"",0,"L");$pdf->SetXY($w+13,$h);$pdf->MultiCell($_PC_WIDTH,6.2,$_TXT,0,"J");$h+=8.2;$h+=(8.2*1.7);
	        $pdf->SetFont('arial','',10);    
		     //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		     $pdf->SetXY($_START_LEFT+(13*2),$h);$pdf->MultiCell($_PC_WIDTH-(13*1),6.2,"identification of the scope of a typical SEO site or cloud environment audit and assessment;",0,"J");$h+=6.2;$h+=(6.2*1);
		     //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		     $pdf->SetXY($_START_LEFT+(13*2),$h);$pdf->MultiCell($_PC_WIDTH-(13*1),6.2,"use of Artificial intelligence (AI) technics to identify specific customer's business needs;",0,"J");$h+=6.2;$h+=(6.2*0);
		     //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		     $pdf->SetXY($_START_LEFT+(13*2),$h);$pdf->MultiCell($_PC_WIDTH-(13*1),6.2,"use of Machine Learning (ML) technics to interpret those specific needs into measurable and actionable SEO goals;",0,"J");$h+=8.2;$h+=(6.2*1);
		     //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		     $pdf->SetXY($_START_LEFT+(13*2),$h);$pdf->MultiCell($_PC_WIDTH-(13*1),6.2,"evaluation of the site or cloud system SEO performance with actionable reviews; lastly",0,"J");$h+=6.2;$h+=(6.2*0);
		     //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		     $pdf->SetXY($_START_LEFT+(13*2),$h);$pdf->MultiCell($_PC_WIDTH-(13*1),6.2,"compilation of a very simple but comprehensive report with recommendations.",0,"J");$h+=6.2;$h+=(6.2*0);
		...
		$pdf->SetFont('arial','',10);       
		    //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		    $pdf->SetXY($_START_LEFT+13+13,$h);$pdf->MultiCell($_PC_WIDTH-13,6.2,"identifies the site's compelling keyword phrases;",0,"J");$h+=6.2;$h+=(6.2*0);
		    //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		    $pdf->SetXY($_START_LEFT+13+13,$h); $pdf->MultiCell($_PC_WIDTH-13,6.2,"plans a simple content strategy based on insights obtained from the analysis process;",0,"J");$h+=6.2;$h+=(6.2*0);
		    //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		    $pdf->SetXY($_START_LEFT+13+13,$h); $pdf->MultiCell($_PC_WIDTH-13,6.2,"creates a simple content strategy giving your business a competitive advangage over your competitors.",0,"J");$h+=6.2;$h+=(6.2*0);
		...
		$pdf->SetFont('arial','',10);       
		    //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		    $pdf->SetXY($_START_LEFT+13+13,$h);$pdf->MultiCell($_PC_WIDTH-13,6.2,"identifies the site structure and functioanl aspects that influnce crawlers;",0,"J");$h+=6.2;$h+=(6.2*0);
		    //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		    $pdf->SetXY($_START_LEFT+13+13,$h); $pdf->MultiCell($_PC_WIDTH-13,6.2,"analyses sitemaps to develop a SEO clear picture;",0,"J");$h+=6.2;$h+=(6.2*0);
		    //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		    $pdf->SetXY($_START_LEFT+13+13,$h); $pdf->MultiCell($_PC_WIDTH-13,6.2,"analyses usability of the site and not overlooking its human friendliness;",0,"J");$h+=6.2;$h+=(6.2*0);
		    //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		    $pdf->SetXY($_START_LEFT+13+13,$h); $pdf->MultiCell($_PC_WIDTH-13,6.2,"weighs the overall site architecure againts a typical search engine crawl budget taking into account the crawl rate limit and crawl health in particular;",0,"J");$h+=6.2;$h+=(6.2*1);
		    //$pdf->Circle($w+(13*1.5),$h+3,1.0,'D');
		    $pdf->SetXY($_START_LEFT+13+13,$h); $pdf->MultiCell($_PC_WIDTH-13,6.2,"makes recommendations on organic optimization strategy that could improve ranking, inbound marketing and drives traffic.",0,"J");$h+=6.2;$h+=(6.2*0);
		...
		$pdf->SetFont('arial','',10);
		    if(isset($_MATRIX_RESULTS[1]["status"])){$pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[1]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'','');$pdf->SetXY($_START_LEFT+13,$h);$pdf->Cell(130,10,"URL",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[2]["status"])){$pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[2]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'','');$pdf->SetXY($_START_LEFT+13,$h);$pdf->Cell(130,10,"Favicon",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[3]["status"])){$pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[3]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'','');$pdf->SetXY($_START_LEFT+13,$h);$pdf->Cell(130,10,"Custom 404 Page",0,"L");$h+=10;}
		...
		$pdf->SetFont('arial','',10);    
		    if(isset($_MATRIX_RESULTS[4]["status"])){$pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[4]["status"].".png",$w+5.5,$h+2.5,6.0,0,'','');$pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"HTML Title Tag",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[5]["status"])){$pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[5]["status"].".png",$w+5.5,$h+2.5,6.0,0,'','');$pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Meta Description Tag",0,"L");$h+=6;$h+=6*2;}
		...
		$pdf->SetFont('arial','',10);  
		    if(isset($_MATRIX_RESULTS[8]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[8]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Language",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[9]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[9]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"HTML Headings",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[21]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[21]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"In-Page Links",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[23]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[23]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Natural Language Processing (NLP)",0,"L");$h+=10;}
		...
		$pdf->SetFont('arial','',10);  
		   if(isset($_MATRIX_RESULTS[20]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[20]["status"].".png",$w+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"URL Resolve",0,"L");$h+=6;}
		   if(isset($_MATRIX_RESULTS[25]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[25]["status"].".png",$w+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"URL Underscores",0,"L");$h+=6;}
		   if(isset($_MATRIX_RESULTS[24]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[24]["status"].".png",$w+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Robots.txt",0,"L");$h+=6;$h+=6*4;}
		...
		$pdf->SetFont('arial','',10);  
		    if(isset($_MATRIX_RESULTS[15]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[15]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Server Uptime",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[14]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[14]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Server IP Address",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[12]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[12]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Doctype",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[13]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[13]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Encoding",0,"L");$h+=10;}
		...
		$pdf->SetFont('arial','',10);  
		    if(isset($_MATRIX_RESULTS[16]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[16]["status"].".png",$w+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"SSL Security",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[17]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[17]["status"].".png",$w+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Mixed Content",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[18]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[18]["status"].".png",$w+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Relative URLs",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[19]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[19]["status"].".png",$w+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Post Covid-19 Cyber Security Policy",0,"L");$h+=6;}
		    if(isset($_MATRIX_RESULTS[22]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[22]["status"].".png",$w+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Information Security Policy",0,"L");$h+=6;$h+=6*1;}
		...
		$pdf->SetFont('arial','',10);  
		    if(isset($_MATRIX_RESULTS[6]["status"])){ $pdf->Image(ROOTPATH."images/iconography/".$_MATRIX_RESULTS[6]["status"].".png",$_START_LEFT+5.5,$h+2.5,6.0,0,'',''); $pdf->SetXY($w+13,$h);$pdf->Cell(130,10,"Traffic Rank",0,"L");$h+=6;$h+=6*1.5;}
		...
		// --- Generic
	       ...
	       $pdf->SetTextColor(255);$pdf->Rect($w+1+15+133.5,$h-16+6,75,6,1,'F');$pdf->Text($w+4+15+133.5,$h+8.0-20+6,"Impact Analysis");
	       $pdf->SetTextColor(65);
	       
		//print_r($_S_DESC);print_r($_MATRIX_RESULTS[$_ruleNo]);
	       $_ruleNo = 1;$_ruleName = "URL"; // --- --- --- RULE #1: BRANDING - URL 
	       $_STATUS = isset($_MATRIX_RESULTS[$_ruleNo]["status"]) && isset($_S_DESC[$_MATRIX_RESULTS[$_ruleNo]["status"]]) ? $_S_DESC[$_MATRIX_RESULTS[$_ruleNo]["status"]] : 0;
	       $_BG_SUBTRACT = (isset($_STATUS) && ($_STATUS==3)) ? 15 : 0;
	       $pdf->SetDrawColor($C_A['r'][$_STATUS]-55,$C_A['g'][$_STATUS]-55,$C_A['b'][$_STATUS]-55);$pdf->SetFillColor($C_A['r'][$_STATUS]+165-$_BG_SUBTRACT,$C_A['g'][$_STATUS]+165-$_BG_SUBTRACT,$C_A['b'][$_STATUS]+165-$_BG_SUBTRACT);$pdf->SetLineWidth(0.3);
	        $pdf->Image(ROOTPATH."images/iconography/".$S_DESC[$_STATUS].".png",$w+6,$h+0.0,8,0,'',''); $pdf->SetFont('arial','',10);$pdf->SetXY($w+13,$h);$pdf->MultiCell($_WIDTH,6," ".$_ruleName,"B","L",true);$h+=6*1.25;
		
	       $pdf->SetLineWidth(0.1);$pdf->SetDrawColor(195);//$pdf->SetDash(1,1);
	       $pdf->Line($w+150,$h-6*1.25+1.5,$w+150,$h-6*1.25+4.5);//$pdf->SetDash();
	       ...
	       $_STATUS = isset($_MATRIX_RESULTS[$_ruleNo]["status"]) && isset($_S_DESC[$_MATRIX_RESULTS[$_ruleNo]["status"]]) ? $_S_DESC[$_MATRIX_RESULTS[$_ruleNo]["status"]] : 0;$_BG_SUBTRACT = (isset($_STATUS) && ($_STATUS==3)) ? 15 : 0;
	       $pdf->SetDrawColor($C_A['r'][$_STATUS]-55,$C_A['g'][$_STATUS]-55,$C_A['b'][$_STATUS]-55);$pdf->SetFillColor($C_A['r'][$_STATUS]+165-$_BG_SUBTRACT,$C_A['g'][$_STATUS]+165-$_BG_SUBTRACT,$C_A['b'][$_STATUS]+165-$_BG_SUBTRACT);$pdf->SetLineWidth(0.3);
	        $pdf->Image(ROOTPATH."images/iconography/".$S_DESC[$_STATUS].".png",$w+6,$h+0.0,8,0,'',''); $pdf->SetFont('arial','',10);$pdf->SetXY($w+13,$h);$pdf->MultiCell($_WIDTH,6," ".$_ruleName,"B","L",true);$h+=6*1.25;
	       ...
	       $pdf->SetLineWidth(0.1);$pdf->SetDrawColor(195);
	       //$pdf->SetDash(1,1);
	       $pdf->Line($w+150,$h-6*1.25+1.5,$w+150,$h-6*1.25+4.5);
	       //$pdf->SetDash();
	       ...
	       $pdf->SetFont('arial','',11);$pdf->SetFillColor(75);
	       $pdf->SetTextColor(255);$pdf->Rect($w+1+10+83.5,$h-16+6,100,6,1,'F');$pdf->Text($w+4+10+83.5,$h+8.0-20+6,"Security Domain");
	       $pdf->SetTextColor(65);
	       ...
	       $pdf->SetLineWidth(0.1);$pdf->SetDrawColor(195);//$pdf->SetDash(1,1);
	       $pdf->Line($w+95,$h-6*1.25+1.5,$w+95,$h-6*1.25+4.5);$pdf->Line($w+150,$h-6*1.25+1.5,$w+150,$h-6*1.25+4.5);//$pdf->SetDash();
		...
		*/
		...
		$pdf->Rect(0,0,$_WIDTH/2,$_HEIGHT,0,'F');
		...
		//$pdf->Image(ROOTPATH."images/icons/message-16.png",$w+1-5,$h+184.5-28*0.715,5,0,'','');$pdf->Text($w+7.5-5,$h+188.20-28*0.735,"oranky@spiderblackonline.co.za");$h +=8*1.0;
		...
		$__TXT .= "\n\n";
		$pdf->SetTextColor(205);$pdf->SetFont('arial','',15);$pdf->Rect($center-40,$h+5,42.5*2,8,1,'F');$pdf->Text($w+2,$h+11.0,"SEO: Search Engine Optimization");$h +=13.0;
