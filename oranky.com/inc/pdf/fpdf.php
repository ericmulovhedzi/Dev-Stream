...
function SetFont($family, $style='', $size=0)
{
	...
	if(!isset($this->fonts[$fontkey]))
	{
		...
		if($family=='arial')
			$family = 'helvetica';
                        if(isset($this->CoreFonts) && in_array($this->CoreFonts))
                        {
	...
	if(isset($this->k) && ($this->k>0)){$this->FontSize = $size/$this->k;}
	$this->CurrentFont = &$this->fonts[$fontkey];
	if(isset($this->page) && ($this->page>0) && isset($this->CurrentFont['i']))
...
function MultiCell($w, $h, $txt, $border=0, $align='J', $fill=false)
{
	...
	if($w==0)
		$w = $this->w-$this->rMargin-$this->x;
	if(isset($this->cMargin)){$wmax = ($w-2*$this->cMargin)*1000/$this->FontSize;}
	$s = str_replace("\r",'',$txt);
	$nb = strlen($s);
	if($nb>0 && $s[$nb-1]=="\n")
		$nb--;
...
function Image($file, $x=null, $y=null, $w=0, $h=0, $type='', $link='')
{
	...
	if(!isset($this->images[$file]))
	{
		...
		$info = $this->$mtd($file);
		if(isset($this->images)){$info['i'] = count($this->images)+1;}
	...
	if($x===null)
		$x = $this->x;
	if(isset($info['i'])){$this->_out(sprintf('q %.2F 0 0 %.2F %.2F %.2F cm /I%d Do Q',$w*$this->k,$h*$this->k,$x*$this->k,($this->h-($y+$h))*$this->k,$info['i']));}
	if($link)
...
function _beginpage($orientation, $size)
{
	...
	if($size=='')
        ...
        if(isset($this->DefPageSize) && is_array($this->DefPageSize)){
...
function _putpages()
{
	...
	{
		if(isset($this->DefPageSize[1]))$wPt = $this->DefPageSize[1]*$this->k;
		if(isset($this->DefPageSize[0]))$hPt = $this->DefPageSize[0]*$this->k;
	}
	...
	$this->_out($kids.']');
	$this->_out('/Count '.$nb);
	if(isset($wPt) && isset($hPt))$this->_out(sprintf('/MediaBox [0 0 %.2F %.2F]',$wPt,$hPt));
	$this->_out('>>');
	$this->_out('endobj');
}
...
{
	$nf = $this->n;
	if(isset($this->diffs) && is_array($this->diffs)){foreach($this->diffs as $diff)
	{
		// Encodings
		$this->_newobj();
	...
	if(isset($this->FontFiles) && is_array($this->FontFiles)){foreach($this->FontFiles as $file=>$info)
	{
		// Font file embedding
		$this->_newobj();
	...
	if(isset($this->fonts) && is_array($this->fonts)){foreach($this->fonts as $k=>$font)
	{
		// Font objects
		...
		else
		{
			...
			$mtd = isset($type) ? '_put'.strtolower($type) : '_put';
			//if(!method_exists($this,$mtd))
...
{
	if(isset($this->images) && is_array($this->images)){foreach($this->images as $image)
		if(isset($image['i']) && isset($image['n'])){$this->_out('/I'.$image['i'].' '.$image['n'].' 0 R');}}
}
...
function _putresourcedict()
{
	...
	$this->_out('/Font <<');
	if(isset($this->fonts) && is_array($this->fonts)){foreach($this->fonts as $font)
		if(isset($font['i'])){$this->_out('/F'.$font['i'].' '.$font['n'].' 0 R');}}
	$this->_out('>>');
	$this->_out('/XObject <<');
...
// --- Ellipse
    ...
    function _SetAlpha($alpha, $bm='Normal')
    {
        // set alpha for stroking (CA) and non-stroking (ca) operations
        $gs = $this->AddExtGState(array('ca'=>$alpha, 'CA'=>$alpha, 'BM'=>'/'.$bm));
        $this->SetExtGState($gs);
