...
if(isset($_SESSION['accesses']->_login['id']) && (count($_SESSION['accesses']->_login) >= 1) && isset($_GET['_module']) && ($_GET['_module'] >= 1) && isset($_GET['_employee']) && ($_GET['_employee'] >= 1))
{
	...
	$_body = "
		...
		<tr>
			...
			<td style='border-bottom:1px solid #ccc;font-size:11px;background:#e3e3e3;line-height:23px;color:#DB0000;text-align:left;'>&nbsp;<b>Department</b></td>
			...
			<td style='border-bottom:1px solid #ccc;font-size:11px;background:#e3e3e3;line-height:23px;color:#DB0000;text-align:center;'>&nbsp;<b>End Time</b></td>
				<td style='background:#e3e3e3;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='center'>&nbsp;Working Hours&nbsp;</td>
	...
	$rs =$db->Execute("
			  SELECT
				DATE_FORMAT(`A`.date,'%d/%m/%Y') AS `date`,DAY(`A`.date) AS `day`,DATE_FORMAT(`A`.date,'%W') AS `day_formated`,`A`.`1051` AS `name`,  
				
				DATE_FORMAT(`A`.date,'%h:%i %p') AS `time_in`,
				DATE_FORMAT(`A`.date,'%h:%i %p') AS `time_out`,
				DATE_FORMAT(`A`.date,'%h:%i %p') AS `islegit`,
				`A`.`1058` AS `_islate`,`A`.`1052` AS `_department`,`A`.`1053` AS `_status`
	...
	if(($rs) && ($rs->numRows() >= 1))
	{
		...
		while(!$rs->EOF)
		{
			...
			$_body .= "
				<tr>
					<td style='border-bottom:1px solid #ccc;font-size:11px;color:#008000;background:#f3f3f3;line-height:23px;font-weight:bold;' align='left'>&nbsp;".$rs->fields['name']."&nbsp;</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='center'><b>".$rs->fields['_department']."</b>&nbsp;</td>";
				
				
			$_body .= "
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='center'><b>".$rs->fields['_status']."</b>&nbsp;</td>
					...
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#c3daef;' align='center'><b></b>".$rs->fields['date']."</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#FBE3E4;' align='center'><b></b>&nbsp;</td>
					
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#E6EFC2;' align='center'><b></b>&nbsp;</td>
					";
	...
	if(!(isset($_GET['_disp']) && ($_GET['_disp'] >= 1)))
	{
		...
		{
			$_body = "<center><a style='text-decoration:none;' target='_blank' href='../../exports/pdf/campaigns/tmp/".$fname." - ".$_GET['_module']." - ".date("Y-F-d").".pdf'><span style='background-color:#ddd;border:2px solid #DB0000;padding:3px;color:#DB0000;-webkit-border-radius:4px;-moz-border-radius:4px;border-radius:4px;font-size:12px;font-weight:bold;'>Download PDF Time & Attendance</span></a> 
			".$_body;
		}
