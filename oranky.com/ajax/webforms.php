...
if(isset($_POST['_c']) && (!empty($_POST['_c'])))
{
  if(isset($_POST['_d']) && (!empty($_POST['_d'])))
  {
    ...
    {
      $url = "http://oefimboboapp.net/webservice/rest/post/mod_service_web_logger.php?_ovhkey=".urlencode(base64_encode(base64_encode(urldecode($_POST['_c'])))).'&_val='.urlencode(base64_encode($dataStr));
	...
	if(isset($rsArr[0]['status']) && ($rsArr[0]['status']>=1) && isset($rsArr[0]['data']) && ($rsArr[0]['data']>=1))
	{
	  ...
	  $_uriExtra['_terms'] = 1;
	  $url = "http://oefimboboapp.net/webservice/rest/post/mod_service_web_logger.php?_ovhkey=".urlencode(base64_encode(base64_encode(urldecode($_POST['_c']))))."&isnew=".$_ISNEW."&_terms=1";
      ...
      else{$array = array("data"=>"3","status"=>0,"description"=>"Please check that you are connected to the internet.".$rsJson);}
    }
