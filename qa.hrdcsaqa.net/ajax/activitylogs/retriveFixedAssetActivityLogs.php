...
if(isset($_SESSION['accesses']->_login['id']) && (count($_SESSION['accesses']->_login) >= 1) && isset($_GET['_module']) && ($_GET['_module'] >= 1))
{
	...
	$fname = "Fixed Asset Register - ".$_MODULE_NAME[1];
	
	$_XLS_DOC = array(array(""), array("Reporting on Module: ",$_MODULE_NAME[1]), array("Parent: ",$_MODULE_NAME[2]), array("Fixed Asset of",$_FIELD_NAME),array(""));
	...
	$_body = "
		<tr>
			<td colspan='$colspan' style='background:#eaeaea;text-align:right;line-height:25px;height:25px;border-bottom:1px solid #ccc;color:#aaa;text-indent:2px;font-size:12px;font-weight:normal;'><i>Reporting on Module: <span style='color:#555;'>Fixed Asset Register</span> of Parent <span style='color:#555;'></span></i>&nbsp;</td>	     
		</tr>
		<tr>
		...
		<tr>
			<td style='border-bottom:1px solid #ccc;font-size:11px;background:#e3e3e3;line-height:23px;color:#DB0000;text-align:left;'>&nbsp;<b>Asset Name</b></td>
			<td style='border-bottom:1px solid #ccc;font-size:11px;background:#e3e3e3;line-height:23px;color:#DB0000;text-align:center;'>&nbsp;<b>Asset Serial No.</b></td>
			<td style='border-bottom:1px solid #ccc;font-size:11px;background:#e3e3e3;line-height:23px;color:#DB0000;text-align:center;'>&nbsp;<b>Asset Barcode</b></td>";
		
		$_body .= "
				<td style='background:#e3e3e3;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='center'>&nbsp;Classification&nbsp;</td>
				<td style='background:#e3e3e3;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='left'>&nbsp;Location&nbsp;</td>";
	...
	$_body .= "
			<td style='background:#c3daef;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='center'>Status</td>
			<td style='background:#FBE3E4;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='center'>Status Date</td>	
			<td style='background:#e3e3e3;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='center'>History</td>
			<td style='background:#E6EFC2;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='center'>&nbsp;Purchase Price&nbsp;</td>
		</tr>";
		
	$_XLS_DOC[] = $_XLS_DOC_TMP;$_XLS_DOC_TMP = array();$_XLS_STYLE[] = $_XLS_STYLE_TMP;$_XLS_STYLE_TMP = array();//= "hdr_bold_fz10_bgred_colorred";
	
	$_TOTAL = $_TOTAL_MACHINERY_EQUIPMENT = $_TOTAL_FURNITURE_FIXTURES = $_TOTAL_PTOPERTIES = $_TOTAL_VEHICLES = $_TOTAL_COMPUTERS = 0;
	
	$_TOTAL_LOCATION = array();
	...
	/*$rs__inv = $db->Execute("SELECT
				`A`.id,`A`.asset_name,`A`.asset_description,`A`.`asset_classification`,`A`.`asset_location`,
							
							`A`.`asset_barcode`,`A`.`asset_serial_no`,
							
							`A`.`asset_purchase_price`,`A`.`asset_purchase_date`,
							`A`.`asset_expiry_date`,v`asset_expiry_value`,
							
							`A`.`status`,`A`.`status_update`
				FROM `_accounting_360_".$_SESSION['accesses']->_login['hr360'][0]."_fixed_assets` AS `A`
	...
	$rs =$db->Execute("
			  SELECT `A`.id
				,`A`.asset_id,`A`.asset_name,`A`.asset_description,`A`.`asset_classification`,`A`.`asset_location`,
							
							`A`.`asset_barcode`,`A`.`asset_serial_no`,
							
							`A`.`asset_purchase_price`,`A`.`asset_purchase_date`,
							`A`.`asset_expiry_date`,`asset_expiry_value`,
							
							`A`.`status`,`A`.`status_update`
			  FROM `_accounting_360_".$_SESSION['accesses']->_login['hr360'][0]."_fixed_assets` AS `A`
			  LEFT JOIN `lists` AS  `B` ON `B`.id=`A`.`asset_location`
			  WHERE `A`.`pub`=1 AND `A`.`del`=1 AND 1=1 ORDER BY `B`.weight,`A`.`asset_location` DESC
			  ");
	if(($rs) && ($rs->numRows() >= 1))
	{
		//$_binLocatinsArr = genericItemsArr("lists"," ");
		
	$_classificationArr = genericItemsArr("lists"," AND `category`=47 ");//print_r($_classificationArr);
	$_locationArr = genericItemsArr("lists"," AND `category`=49 ");//print_r($_classificationArr);	
	$_locationDescArr = genericItemsArr("lists"," AND `category`=49 ",array("id","description"));//print_r($_classificationArr);
	$_locationWeightArr = genericItemsArr("lists"," AND `category`=49 ",array("id","weight"));//print_r($_classificationArr);
		...
		while(!$rs->EOF)
		{
			...
			$itemName = $rs->fields['asset_name'];
			
			$location = isset($_locationArr[$rs->fields['asset_location']]) ? $_locationArr[$rs->fields['asset_location']] : "";
			$classification = isset($_classificationArr[$rs->fields['asset_classification']]) ? $_classificationArr[$rs->fields['asset_classification']] : "";
			
			$rs_sub =$db->Execute("SELECT COUNT(id) AS num FROM `_accounting_360_".$_SESSION['accesses']->_login['hr360'][0]."_fixed_assets_history` WHERE `asset_id`='".$rs->fields['id']."' AND pub='1' AND del='1' GROUP BY `asset_id`");
			$subitems = 0;if(($rs_sub) && ($rs_sub->numRows() >= 1)){$subitems = $rs_sub->fields['num'];}if($subitems == 0) $subitems = 0;
			...
			$_body .= "
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='center'><b>".$rs->fields['asset_serial_no']."</b>&nbsp;</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='center'><b>".$rs->fields['asset_barcode']."</b>&nbsp;</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='center'><b>".$classification."</b>&nbsp;</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='left'><b>&nbsp;".$location."</b>&nbsp;</td>
					
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#c3daef;' align='center'><b>".$rs->fields['status']."</b>&nbsp;</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#FBE3E4;' align='center'><b>".$rs->fields['status_update']."</b>&nbsp;</td>
					
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='center'><b>".$subitems."</b>&nbsp;</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#E6EFC2;' align='right'><b>R ".$rs->fields['asset_purchase_price']."</b></td>
					";
					
			$_TOTAL = $_TOTAL + ($rs->fields['asset_purchase_price']);
			//echo $classification;
			$pos = strpos($classification, "MACHINERY");$pos1 = strpos($classification, "EQUIPMENT");
			if(($pos !== false) || ($pos1 !== false)){$_TOTAL_MACHINERY_EQUIPMENT = $_TOTAL_MACHINERY_EQUIPMENT + ($rs->fields['asset_purchase_price']);}
							
			//if($classification){$_TOTAL_MACHINERY_EQUIPMENT = $_TOTAL_MACHINERY_EQUIPMENT + ($rs->fields['asset_purchase_price']);}
							
			
			$pos2 = strpos($classification, "VEHICLE");$pos3 = strpos($classification, "VEHICLE");
			if(($pos2 !== false) || ($pos3 !== false)){$_TOTAL_VEHICLES = $_TOTAL_VEHICLES + ($rs->fields['asset_purchase_price']);}
							
			//if($classification){$_TOTAL_MACHINERY_EQUIPMENT = $_TOTAL_MACHINERY_EQUIPMENT + ($rs->fields['asset_purchase_price']);}
				
			if(isset($_TOTAL_LOCATION[$rs->fields['asset_location']]))
			{
				if(($pos !== false) || ($pos1 !== false)){$_TOTAL_LOCATION[$rs->fields['asset_location']][0] = $_TOTAL_LOCATION[$rs->fields['asset_location']][0] + $rs->fields['asset_purchase_price'];}
				if(($pos !== false) || ($pos1 !== false)){$_TOTAL_LOCATION[$rs->fields['asset_location']][1] = $_TOTAL_LOCATION[$rs->fields['asset_location']][1] + 1;}
				if(($pos2 !== false) || ($pos3 !== false)){$_TOTAL_LOCATION[$rs->fields['asset_location']][2] = $_TOTAL_LOCATION[$rs->fields['asset_location']][2] + $rs->fields['asset_purchase_price'];}
				if(($pos2 !== false) || ($pos3 !== false)){$_TOTAL_LOCATION[$rs->fields['asset_location']][4] = $rs->fields['asset_id'];}
				
				$_TOTAL_LOCATION[$rs->fields['asset_location']][3] = $_TOTAL_LOCATION[$rs->fields['asset_location']][3] + $rs->fields['asset_purchase_price'];
			}
			else
			{
				if(($pos !== false) || ($pos1 !== false)){$_TOTAL_LOCATION[$rs->fields['asset_location']][0] = $rs->fields['asset_purchase_price'];}
				if(($pos !== false) || ($pos1 !== false)){$_TOTAL_LOCATION[$rs->fields['asset_location']][1] = 1;}
				if(($pos2 !== false) || ($pos3 !== false)){$_TOTAL_LOCATION[$rs->fields['asset_location']][2] = $rs->fields['asset_purchase_price'];}
				if(($pos2 !== false) || ($pos3 !== false)){$_TOTAL_LOCATION[$rs->fields['asset_location']][4] = $rs->fields['asset_id'];}
				
				$_TOTAL_LOCATION[$rs->fields['asset_location']][3] = $rs->fields['asset_purchase_price'];
			}
			
			$totalArr['reject'] += $rs->fields['reject'];
			$totalArr['approve'] += $rs->fields['approve'];
			$totalArr['total'] += $rs->fields['asset_purchase_price'];
			...
			$_DATA_ARR[] = array(substr($itemName,0,18),substr($rs->fields['asset_serial_no'],0,18),substr($rs->fields['asset_barcode'],0,12),substr($classification,0,12),substr($location,0,12),$rs->fields['status'],$rs->fields['status_update'],$subitems,$rs->fields['asset_purchase_price']);
	...
	}else{echo $db->errorMsg();}
	
	$fname = "Fixed Asset Register - House of Nnyane";
	...
	if((isset($_GET['_isemail']) && ($_GET['_isemail'] >= 1)) || (isset($_GET['is_excel']) && ($_GET['is_excel'] >= 1)) || (isset($_GET['is_pdf']) && ($_GET['is_pdf'] >= 1)))
	{
		...
		// _____________________________________________________ Page # 1 - Cover Page __________________________________________________
				...
				$pdf->SetXY($w+2+($_P_WIDTH/2.5),$h+9.5);$pdf->Cell($_P_WIDTH/2.5,6,"FIXED ASSET","",0,"C",false);
				$pdf->SetFont('arial','',11);$pdf->SetTextColor(135);$pdf->SetXY($w+3.5+($_P_WIDTH/2.5),$h+17.55);$pdf->Cell($_P_WIDTH/2.61,6,"( Total Fixed Asset Asset Value ) ",0,0,"C",false);
				
				$h = $_START_TOP;$w = $_START_LEFT;
				
				$pdf->SetTextColor(0);$pdf->SetFont('arial','',34);
				
				$pdf->SetXY(70-2,$h+1.85);$pdf->MultiCell($_HEIGHT,15,"Fixed Asset Register",0,"L");
				...
				//$pdf->SetTextColor(100);$pdf->SetFont('arial','',18);$pdf->Text($xPos,$yPos,"Fixed Asset Overview");$yPos+=8;
				...
				$pdf->SetFont('arial','',10);
					...
					$pdf->SetXY($w,$h);$pdf->MultiCell(50,6,"Asset Name",1,'L',true);
					$pdf->SetXY($w+50,$h);$pdf->MultiCell(40,6,"Engraved Asset Serial No.",1,'C',true);
					$pdf->SetXY($w+50+40,$h);$pdf->MultiCell(30,6,"Asset Barcode",1,'L',true);
					$pdf->SetXY($w+50+40+30,$h);$pdf->MultiCell(30,6,"Classification",1,'C',true);
					$pdf->SetXY($w+50+40+25+35,$h);$pdf->MultiCell(30,6,"Current Location",1,'L',true);
					$pdf->SetXY($w+50+40+15+25+20+30,$h);$pdf->MultiCell(25,6,"Status",1,'R',true);
					$pdf->SetXY($w+50+40+15+20+20+30+30,$h);$pdf->MultiCell(25,6,"Status Date",1,'C',true);
					$pdf->SetXY($w+50+40+15+20+20+30+30+25,$h);$pdf->MultiCell(20,6,"History",1,'C',true);
					$pdf->SetXY($w+50+40+15+20+20+30+30+30+15,$h);$pdf->MultiCell(30,6,"Purchase Price",1,'R',true);
				...
				foreach($_DATA_ARR as $k=>$v)
				{
					...
					$pdf->SetTextColor(0);$pdf->SetFillColor(255);
					    ...
					    $pdf->SetXY($w+50+40+15+25+20+30,$h);$pdf->MultiCell(30,5,$v[5],1,'C');
					    $pdf->SetFillColor(244,188,191);
					    $pdf->SetXY($w+50+40+15+20+20+30+30,$h);$pdf->MultiCell(25,5,$v[6],1,'C',true);
					    $pdf->SetFillColor(128,246,166);
					    $pdf->SetXY($w+50+40+15+20+20+30+30+25,$h);$pdf->MultiCell(20,5,$v[7],1,'C');$pdf->SetXY($w+50+40+15+20+20+30+30+30+15,$h);$pdf->MultiCell(30,5,"".$v[8],1,'R',true);
				...
				$pdf->SetXY($w+180,$h);$pdf->MultiCell(70,7,"Total Fixed Asset Value:",1,'R',true);
				$pdf->SetFillColor(255);$pdf->SetTextColor(245,0,0);$pdf->SetFont('arial','B',10);
		...
		// _____________________________________________________ Page # 1 - Cover Page __________________________________________________
				...
				//$pdf->SetAlpha(0.25);$pdf->SetXY($w+2+($_P_WIDTH/2.5),$h);$pdf->MultiCell($_P_WIDTH/2.5,24,"","","C",true);$pdf->SetAlpha(1);
				$pdf->SetXY($w+2+($_P_WIDTH/2.5),$h+24);$pdf->MultiCell($_P_WIDTH/2.5,2.5,"","","C",true);
				$pdf->SetXY($w+2+($_P_WIDTH/2.5),$h+9.5);$pdf->Cell($_P_WIDTH/2.5,6,"PROJECTS REPORT","",0,"C",false);
				$pdf->SetFont('arial','',11);$pdf->SetTextColor(135);$pdf->SetXY($w+3.5+($_P_WIDTH/2.5),$h+17.55);$pdf->Cell($_P_WIDTH/2.61,6,"( Asset Asset Value per Project) ",0,0,"C",false);
				
				$h = $_START_TOP;$w = $_START_LEFT;
				
				$pdf->SetTextColor(0);$pdf->SetFont('arial','',34);
				
				$pdf->SetXY(70-2,$h+1.85);$pdf->MultiCell($_HEIGHT,15,"Asset Assigned to Active \nProjects:",0,"L");
				$pdf->SetTextColor(155);
				//$pdf->SetXY(70-2,$h+16.85);$pdf->MultiCell($_HEIGHT,15,"(Stock on hand)",0,"L");
				...
				$pdf->SetFont('arial','',10);
					...
					//$h+=6;
					$pdf->SetFillColor(25);$pdf->SetTextColor(255);$pdf->SetFont('arial','',10);
					$pdf->SetXY($w,$h);$pdf->MultiCell(50+20,6,"Project Name",1,'L',true);
					//$pdf->SetXY($w+50,$h);$pdf->MultiCell(40,6,"Engraved Asset Serial No.",1,'C',true);
					$pdf->SetXY($w+50+20,$h);$pdf->MultiCell(25,6,"Status",1,'C',true);
					$pdf->SetXY($w+50+40+5,$h);$pdf->MultiCell(50,6,"Assigned To (Supervisor):",1,'L',true);
					$pdf->SetXY($w+50+40+25+30,$h);$pdf->MultiCell(30+15,6,"Machinery and Equipment",1,'C',true);
					//$pdf->SetXY($w+50+40+15+25+20+30,$h);$pdf->MultiCell(25+25,6,"Machinery and Equipment",1,'R',true);
					$pdf->SetXY($w+50+40+15+20+20+30+30-15,$h);$pdf->MultiCell(35,6,"No. of Items",1,'C',true);
					$pdf->SetXY($w+50+40+15+20+20+30+30+20,$h);$pdf->MultiCell(25,6,"Vehicle",1,'C',true);
					$pdf->SetXY($w+50+40+15+20+20+30+30+30+15,$h);$pdf->MultiCell(30,6,"No. of Items",1,'C',true);
					
					$h+=6;
					$pdf->SetFont('arial','',10);$pdf->SetTextColor(39,44,49);
					
					$_INDEX = 0;$_CUT_OFF = 20;
					foreach($_TOTAL_LOCATION as $kk=>$vv)
					{
						$pdf->SetTextColor(0);$pdf->SetFillColor(255);
						
						$location = isset($_locationArr[$kk]) ? $_locationArr[$kk] : "";
						$assigndTo = isset($_locationDescArr[$kk]) ? $_locationDescArr[$kk] : "";
						$statusP = isset($_locationWeightArr[$kk]) ? $_locationWeightArr[$kk] : "";
						//$location = isset($_locationDescArr[$rs->fields['asset_location']]) ? $_locationArr[$rs->fields['asset_location']] : "";
						
					    $pdf->SetXY($w,$h);$pdf->MultiCell(50+20,5,($_INDEX+1).". ".$location,1,'L',true);//$pdf->SetFont('arial','',9.5);$pdf->SetXY($w+45,$h);$pdf->MultiCell(50,5,$v[1],1,'L',true);$pdf->SetFont('arial','',10);
					    //$pdf->SetXY($w+50+40,$h);$pdf->MultiCell(30,5,$v[2],1,'L',true);
					    $pdf->SetXY($w+50+20,$h);$pdf->MultiCell(25,5,$statusP,1,'C');
					    $pdf->SetXY($w+50+40+5,$h);$pdf->MultiCell(50,5,substr($assigndTo,0,35),1,'L');
					    $pdf->SetXY($w+50+40+25+30,$h);$pdf->MultiCell(30+15,5,"R ".number_format(round($vv[0],2),2,"."," ")." ",1,'R');
					    //$pdf->SetFillColor(244,188,191);
					    $pdf->SetXY($w+50+40+15+20+20+30+15,$h);$pdf->MultiCell(35,5,$vv[1],1,'C',true);
					    //$pdf->SetFillColor(128,246,166);
					    $pdf->SetXY($w+50+40+15+20+20+30+30+20,$h);$pdf->MultiCell(25,5,$vv[4]." ",1,'c');
					    $pdf->SetXY($w+50+40+15+20+20+30+30+30+15,$h);$pdf->MultiCell(30,5," ",1,'R',true);
				...
				$pdf->SetFont('arial','B',10);$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Inventory (Stock)",0,"L");$pdf->SetFont('arial','',10);$pdf->SetTextColor(105);$pdf->SetXY($_START_LEFT+105,$h);$pdf->Cell(30,6,"R0.00",0,"R","R");$pdf->SetFont('arial','B',10);$pdf->SetXY($_START_LEFT+110+30,$h);$pdf->Cell(30,6,"R0.00",0,"R","R");$h+=6;
				$pdf->SetFont('arial','B',10);$pdf->SetTextColor(25);
				$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Total current assets","TB","L");$pdf->SetXY($_START_LEFT+101,$h);$pdf->Cell(34,6,"R0.00","TB","R","R");$pdf->SetFont('arial','B',10);$pdf->SetXY($_START_LEFT+106+30,$h);$pdf->Cell(34,6,"R0.00","TB","R","R");$h+=6;
				...
				$pdf->SetTextColor(245,0,0);$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Vehicles",0,"L");$pdf->SetXY($_START_LEFT+105,$h);$pdf->Cell(30,5,"R0.00",0,"R","R");$pdf->SetXY($_START_LEFT+110+30,$h);$pdf->Cell(30,6,"R ".number_format(round($_TOTAL_VEHICLES,2),2,"."," "),0,"R","R");$h+=6;
				$pdf->SetTextColor(245,0,0);$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Machinery and equipment",0,"L");$pdf->SetXY($_START_LEFT+105,$h);$pdf->Cell(30,6,"R0.00",0,"R","R");$pdf->SetXY($_START_LEFT+110+30,$h);$pdf->Cell(30,6,"R ".number_format(round($_TOTAL_MACHINERY_EQUIPMENT,2),2,"."," "),0,"R","R");$h+=6;
				$pdf->SetTextColor(25);
				...
				$pdf->SetFont('arial','B',10);$pdf->SetTextColor(25);
				$pdf->SetXY($_START_LEFT,$h);$pdf->Cell(100,6,"Total fixed assets","TB","L");$pdf->SetXY($_START_LEFT+101,$h);$pdf->Cell(34,6,"R0.00","TB","R","R");$pdf->SetXY($_START_LEFT+106+30,$h);$pdf->Cell(34,6,"R ".number_format(round($_TOTAL,2),2,"."," "),"TB","R","R");$h+=6;
