...
if(isset($_SESSION['accesses']->_login['id']) && (count($_SESSION['accesses']->_login) >= 1) && isset($_GET['_module']) && ($_GET['_module'] >= 1) && isset($_GET['_employee']) && ($_GET['_employee'] >= 1) && isset($_GET['_date_from']) && (!empty($_GET['_date_from'])) && isset($_GET['_date_to']) && (!empty($_GET['_date_to'])))
{
	...
	$wfArr = $totalArr = $_MODULE_NAME = $_XLS_DOC = $_XLS_STYLE = $_DATA_ARR = array();$stages = $_FIELD_NAME = $_SQL_LIMIT = $_SQL_LIMIT_ASSET = $_SQL_LIMIT_SUB = "" ;$_GET['_schedule'] = (isset($_GET['_schedule']) && ($_GET['_schedule']>=1)) ? $_GET['_schedule'] : 2;
	...
	$_company = 0;
	if(isset($_SESSION['accesses']->_login['accounting360'][0]) && (!empty($_SESSION['accesses']->_login['accounting360'][0]))){$_company = $_SESSION['accesses']->_login['accounting360'][0];}
	
	$_locationArr = genericItemsArr("fixed_asset_locations"," AND `company`=".$_company." ");//print_r($_classificationArr);
	$_locationSupervisorArr = genericItemsArr("fixed_asset_locations"," AND `company`=".$_company." ",array('id','supervisor'));
	$_classificationArr = genericItemsArr("lists"," AND `category`=47 ");//print_r($_classificationArr);
	
	$usersArr = genericItemsArr("_hr_360_".$_SESSION['accesses']->_login['hr360'][0]."_employees");
	$usersSurnamesArr = genericItemsArr("_hr_360_".$_SESSION['accesses']->_login['hr360'][0]."_employees","",array("id","surname"));
	...
	if(isset($_GET['_date_from']) && (!empty($_GET['_date_from'])))
	{
		...
		$_SQL_LIMIT_ASSET .= " AND TO_DAYS(`A`.status_update) >= TO_DAYS('".$_GET['_date_from'] ."') ";
		...
		$_PDF_DISPLAY .= date("jS F, Y",strtotime($_GET['_date_from']));
	}
	...
	if(isset($_GET['_date_to']) && (!empty($_GET['_date_to'])))
	{
		...
		$_SQL_LIMIT_ASSET .= " AND TO_DAYS(`A`.status_update) <= TO_DAYS('".$_GET['_date_to'] ."') ";
		//$_SQL_LIMIT_SUB .= " AND TO_DAYS(`A`.date) <= TO_DAYS('".$_GET['_date_to'] ."') ";
		$_USER_DISPLAY .= " <i style='color:#333;'>on Month: </i> <span style='color:#DD7000;'>".date("F, Y",strtotime("01-".$_GET['_date_to']))."</span>";
		$_PDF_DISPLAY .= " - ".date("jS F, Y",strtotime($_GET['_date_to']));
	}
	else
	{
	...
	{
		$_SQL_LIMIT .= " AND `A`.`storage_location`= '".$_GET['_employee']."' ";
		$_SQL_LIMIT_ASSET .= " AND `A`.`asset_location`= '".$_GET['_employee']."' ";
	}
	...
	$_EXUECTUTE = true;
		...
		//print_r($_locationSupervisorArr);echo $_GET['_employee']."-".$_locationSupervisorArr[$_GET['_employee']];
		$_MODULE_NAME[1] = isset($_locationArr[$_GET['_employee']]) ? $_locationArr[$_GET['_employee']] : "N/A";
		//print_r($usersArr);
		$__name = isset($usersArr[$_locationSupervisorArr[$_GET['_employee']]]) ? $usersArr[$_locationSupervisorArr[$_GET['_employee']]] : "N/A";
		$__surname = isset($usersSurnamesArr[$_locationSupervisorArr[$_GET['_employee']]]) ?$usersSurnamesArr[$_locationSupervisorArr[$_GET['_employee']]] : "N/A";
	
		$_EMPLOYEE_NAME = $__name." ".$__surname;
		
		
	$fname = "Acknowledgement Receipt - ".$_MODULE_NAME[1];
	...
	$_body = "
		<tr>
			<td colspan='$colspan' style='background:#eaeaea;text-align:right;line-height:25px;height:25px;border-bottom:1px solid #ccc;color:#aaa;text-indent:2px;font-size:12px;font-weight:normal;'><i>Reporting on Module: <span style='color:#555;'>Inventory Management</span>.</i>&nbsp;</td>	     
		</tr>
		<tr>
			<td colspan='$colspan' style='background:#eaeaea;text-align:left;line-height:25px;height:25px;border-bottom:1px solid #ccc;color:#053e57;text-indent:2px;font-size:13px;'><b>Acknowledgement Receipt <span style='color:#e62899;'>".$_MODULE_NAME[1]."</span></b></td>	     
		</tr>
		<tr>
			<td style='border-bottom:1px solid #ccc;font-size:11px;background:#e3e3e3;line-height:23px;color:#DB0000;text-align:left;'>&nbsp;<b>Item Code</b></td>
			...
			<td style='border-bottom:1px solid #ccc;font-size:11px;background:#e3e3e3;line-height:23px;color:#DB0000;text-align:left;'>&nbsp;<b>Description </b></td>
			<td style='background:#c3daef;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='center'>Classification</td>
			<td style='background:#c3daef;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='center'>Date</td>
			<td style='background:#e3e3e3;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='center'>&nbsp;QTY&nbsp;</td>
			<td style='background:#e3e3e3;border-bottom:1px solid #ccc;color:#125687;font-weight:bold;font-size:11px;' align='center'>&nbsp;Total Cost&nbsp;</td>";
	...
	$rs =$db->Execute("
			  SELECT
				...
				`A`.date
			  FROM `_accounting_360_".$_SESSION['accesses']->_login['accounting360'][0]."_inventory_sold` AS `A`
			  LEFT JOIN `_accounting_360_".$_SESSION['accesses']->_login['accounting360'][0]."_inventory_sku` AS `B` ON `B`.id=`A`.item 
	...
	if(($rs) && ($rs->numRows() >= 1))
	{
		...
		while(!$rs->EOF)
		{
			...
			$classification = isset($_classificationArr[$rs->fields['item_classification']]) ? $_classificationArr[$rs->fields['item_classification']] : "";
			...
			$_body .= "
				<tr>
					<td style='border-bottom:1px solid #ccc;font-size:11px;color:#008000;background:#f3f3f3;line-height:23px;font-weight:bold;' align='left'>&nbsp;(".$_INDEX.") &nbsp;".$rs->fields['item_id']."&nbsp;</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='left'>&nbsp;".$rs->fields['item_name']."</td>";
				
				
			$_body .= "
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='left'><b></b>&nbsp;</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#c3daef;' align='left'><b>".$classification."</b></td>
					...
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='center'><b>".$rs->fields['qty']."</b>&nbsp;</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#E6EFC2;' align='right'><b>R ".number_format(round($rs->fields['qty_price'],2),2,"."," ")."</b>&nbsp;</td>
					";
	...
	$rs =$db->Execute("
			  SELECT
				...
				`A`.`asset_purchase_price`,
				`A`.`asset_classification`,
				
				`A`.date
			  FROM  `_accounting_360_".$_SESSION['accesses']->_login['accounting360'][0]."_fixed_assets` AS `A` 
			  WHERE `A`.pub=1 AND `A`.del=1 AND `A`.status='IN' ".$_SQL_LIMIT_ASSET."
	...
	if(($rs) && ($rs->numRows() >= 1))
	{
		...
		while(!$rs->EOF)
		{
			...
			$_body .= "
				<tr>
					<td style='border-bottom:1px solid #ccc;font-size:11px;color:#008000;background:#f3f3f3;line-height:23px;font-weight:bold;' align='left'>&nbsp;(".$_INDEX.") &nbsp;".$rs->fields['asset_id']."&nbsp;</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='left'>&nbsp;".$rs->fields['asset_name']."</td>";
			...
			$_body .= "
					...
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#c3daef;' align='left'><b>".$classification."</b></td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#c3daef;' align='center'><b></b>".substr($rs->fields['date'],0,11)."</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#f9f9f9;' align='center'><b>1</b>&nbsp;</td>
					<td style='border-bottom:1px solid #ececec;color:#213552;font-size:11px;background:#E6EFC2;' align='right'><b>R ".number_format(round($rs->fields['asset_purchase_price'],2),2,"."," ")."</b>&nbsp;</td>
					";
			...
			$_DATA_ARR[] = array
					(
					     $rs->fields['asset_id'],
					     $rs->fields['asset_name'],
					     '',
					     $classification,
					     substr($rs->fields['date'],0,11),
					     1,
					     $rs->fields['asset_purchase_price']
					);
	...
	if(!(isset($_GET['_disp']) && ($_GET['_disp'] >= 1)))
	{
		...
		{
			$_body = "<center><a style='text-decoration:none;' target='_blank' href='../../exports/pdf/campaigns/tmp/".$fname." - ".$_GET['_employee']." - ".date("Y-F-d").".pdf'><span style='background-color:#ddd;border:2px solid #008000;padding:3px;color:#008000;-webkit-border-radius:4px;-moz-border-radius:4px;border-radius:4px;font-size:12px;font-weight:bold;'>Download PDF Acknowledgement Receipt Form</span></a> 
			".$_body;
		}
	...
	if((isset($_GET['_isemail']) && ($_GET['_isemail'] >= 1)) || (isset($_GET['is_excel']) && ($_GET['is_excel'] >= 1)) || (isset($_GET['is_pdf']) && ($_GET['is_pdf'] >= 1)))
	{
		...
		//$pdf->Open();
		$pdf = new FPDF('P','mm',array(216,297));
		...
		*/
		...
		$h=0;$w = 73;
		$pdf->SetFillColor(155,155,155);$pdf->SetTextColor(0);$pdf->SetFont('arial','B',38);
		//$pdf->SetAlpha(0.45);
		$pdf->SetXY($w+2+($_P_WIDTH/2.5) - 7.5,$h);$pdf->MultiCell($_P_WIDTH/2.5,24,"","","C",true);
		//$pdf->SetAlpha(1);
		$pdf->SetFillColor(75,75,75);
		$pdf->SetXY($w+2+($_P_WIDTH/2.5) - 7.5,$h+9.0);$pdf->Cell($_P_WIDTH/2.5,6,"Receipt","",0,"C",false);
		$pdf->SetFont('arial','',9);$pdf->SetTextColor(0);$pdf->SetXY($w+3+($_P_WIDTH/2.5) - 7.5,$h+17.75);$pdf->Cell($_P_WIDTH/2.61,6,"( RECEIPT ACKNOWLEDGEMENT FORM )","",0,"C",false);
		$pdf->SetXY($w+2+($_P_WIDTH/2.5) - 7.5,$h+24);$pdf->MultiCell($_P_WIDTH/2.5,2.5,"","","C",true);
		
		$h = $_START_TOP;$w = $_START_LEFT;
		
		    $pdf->SetTextColor(0);$pdf->SetFont('arial','',28);
		
		//$pdf->SetXY(10-2,$h+1.85);$pdf->MultiCell($_HEIGHT,15,"Month: ".date("F Y"),0,"L");
		...
		$h = $_START_TOP;$w = $_START_LEFT;
		    ...
		    $pdf->SetTextColor(0);$pdf->SetFont('arial','',22);
		//$pdf->SetXY(0,$h+1.85);$pdf->MultiCell($_HEIGHT,15,"OVH IoT: Manufacturing \nDivision",0,"C");
		$pdf->SetXY(70-2,$h+1.85);$pdf->MultiCell($_HEIGHT,15,"Acknowledgement\nReceipt Form",0,"L");
		...
		$pdf->SetLineWidth(0.1);$pdf->SetDrawColor(0);$pdf->SetFillColor(0);$pdf->SetTextColor(255);$pdf->SetFont('arial','B',10);
		
		
		$pdf->SetXY($w,$h);$pdf->MultiCell(40,8,"Project Name:",1,'L',true);$pdf->SetTextColor(0);$pdf->SetXY($w+40,$h);$pdf->MultiCell(80,8,$_MODULE_NAME[1],1,'L');$h+=8;
		$h+=6*1;
		
		$pdf->SetLineWidth(0.1);$pdf->SetDrawColor(0);$pdf->SetFillColor(254,247,52);$pdf->SetTextColor(0);$pdf->SetFont('arial','B',10);
		
		
		$pdf->SetXY($w,$h);$pdf->MultiCell(40,6,"Employee Name:",1,'L',true);$pdf->SetXY($w+40,$h);$pdf->MultiCell(80,6,$_EMPLOYEE_NAME,1,'L');$h+=6;
		$pdf->SetXY($w,$h);$pdf->MultiCell(40,6,"Department:",1,'L',true);$pdf->SetXY($w+40,$h);$pdf->MultiCell(80,6,'',1,'L');$h+=6;
		$pdf->SetXY($w,$h);$pdf->MultiCell(40,6,"Date:",1,'L',true);$pdf->SetXY($w+40,$h);$pdf->MultiCell(80,6,$_PDF_DISPLAY,1,'L');$h+=6;
		
		$h+=6*1;
		
		$pdf->SetFont('arial','B',10);
		$pdf->SetXY($w,$h);$pdf->MultiCell(20,6,"Item Code",1,'L',true);
		$pdf->SetXY($w+20,$h);$pdf->MultiCell(50,6,"Description",1,'C',true);
		$pdf->SetXY($w+20+50,$h);$pdf->MultiCell(50,6,"Classification",1,'C',true);
		$pdf->SetXY($w+20+50+50,$h);$pdf->MultiCell(30,6,"Date",1,'C',true);
		$pdf->SetXY($w+20+50+50+30,$h);$pdf->MultiCell(20,6,"Qty",1,'C',true);
		$pdf->SetXY($w+20+50+50+30+20,$h);$pdf->MultiCell(30,6,"Total Cost",1,'C',true);
		...
		$_TOTAL_QTY = $_TOTAL_COST = 0;
				...
				foreach($_DATA_ARR as $k=>$v)
				{
					...
					$pdf->SetXY($w,$h);$pdf->MultiCell(20,5,$v[0],1,'L');$pdf->SetXY($w+20,$h);$pdf->MultiCell(50,5,$v[1],1,'L');
					$pdf->SetXY($w+20+50,$h);$pdf->MultiCell(50,5,$v[3],1,'L');
					$pdf->SetXY($w+20+50+50,$h);$pdf->MultiCell(30,5,$v[4],1,'C');
					$pdf->SetXY($w+20+50+50+30,$h);$pdf->MultiCell(20,5,$v[5],1,'C');
					$pdf->SetXY($w+20+50+50+30+20,$h);$pdf->MultiCell(30,5,"R ".number_format(round($v[6],2),2,"."," "),1,'R');
					
					$_TOTAL_QTY = $_TOTAL_QTY + $v[5];
					$_TOTAL_COST = $_TOTAL_COST + $v[6];
		...
		$pdf->SetFont('arial','',10);$pdf->SetFillColor(245);$pdf->SetFont('arial','B',10);
		$pdf->SetXY($w+20+50+50,$h);$pdf->MultiCell(30,6," Total: ",1,'R');$pdf->SetFont('arial','B',10);$pdf->SetXY($w+20+50+50+30,$h);$pdf->MultiCell(20,6,$_TOTAL_QTY,1,'C',false);
		$pdf->SetXY($w+20+50+50+30+20,$h);$pdf->MultiCell(30,6,"R ".number_format(round($_TOTAL_COST,2),2,"."," "),1,'R',true);
		$h+=6;$pdf->SetFont('arial','',10);
		
		$h+=6*2;$pdf->SetFillColor(255);
		$pdf->SetXY($w,$h);$pdf->MultiCell(40,6,"Received by:",0,'L',true);$pdf->SetXY($w+30,$h-2);$pdf->MultiCell(50,6,ucwords(strtolower($_EMPLOYEE_NAME)),"B",'L',true);
		$h+=7;
		$pdf->SetXY($w,$h);$pdf->MultiCell(40,6,"Signature:",0,'L',true);$pdf->SetXY($w+30,$h-2);$pdf->MultiCell(50,6,"","B",'L',true);
		$h+=7;
				
		// ______________________________________________ Acknowledgement Receipt Cover Page #2 __________________________________________________
		/*
		$pdf->AddPage();$p++;$h = $_START_TOP;$w = $_START_LEFT;
		...
		//$pdf->SetAlpha(1);
		$pdf->SetFillColor(75,75,75);
		$pdf->SetXY($w+2+($_P_WIDTH/2.5) - 7.5,$h+9.0);$pdf->Cell($_P_WIDTH/2.5,6,"Acknowledgement Receipt","",0,"C",false);
		$pdf->SetFont('arial','',11.5);$pdf->SetTextColor(95);$pdf->SetXY($w+3+($_P_WIDTH/2.5) - 7.5,$h+17.75);$pdf->Cell($_P_WIDTH/2.61,6,"( Acknowledgement Receipt Book with 238 Weeks )","",0,"C",false);
		$pdf->SetXY($w+2+($_P_WIDTH/2.5) - 7.5,$h+24);$pdf->MultiCell($_P_WIDTH/2.5,2.5,"","","C",true);
		...
		if(($_rs) && ($_rs->numRows() >= 1))
		{
			...
			//$_HOURS_TOTAL = 0; $_INDEX = 1;
			$___DATA_ARR = array();$_clockkedDateCurrent = "";
			while(!$_rs->EOF)
			...
			if(isset($___DATA_ARR) && is_array($___DATA_ARR))
			{
				...
				foreach ($___DATA_ARR as $k => $v)
				{
					...
					if(isset($v) && is_array($v))
					{
						...
						$pdf->SetXY($w,$h);$pdf->MultiCell(25,6,$v[0][0],1,'C',true);$pdf->SetXY($w+25,$h);$pdf->MultiCell(25,6,$v[0][1],1,'C',true);$pdf->SetXY($w+25+25,$h);$pdf->MultiCell(20,6,$START_TIME,1,'C');$pdf->SetXY($w+25+25+20,$h);$pdf->MultiCell(20,6,$_BREAKS,1,'C');
		...
		$pdf->SetXY(5+$w+25+25+20+20,$h);$pdf->MultiCell(40+30,6," Rate Per Hour",1,'L');$pdf->SetFont('arial','B',10);$pdf->SetXY(5+$w+25+25+20+20+20+20+30,$h);$pdf->MultiCell(30,6,"R".$_EMPLOYEE_RATE,1,'R',true);
		$h+=6;$pdf->SetFont('arial','',10);
		...
		$h+=6;$pdf->SetFont('arial','',10);
		$pdf->SetXY(5+$w+25+25+20+20,$h);$pdf->MultiCell(40+30,6," Total Work Days",1,'L');$pdf->SetFont('arial','B',10);$pdf->SetXY(5+$w+25+25+20+20+20+20+30,$h);$pdf->MultiCell(30,6,"",1,'R',true);
		$h+=6;$pdf->SetFont('arial','',10);
		...
		$h+=6;$pdf->SetFont('arial','',10);
		$pdf->SetXY(5+$w+25+25+20+20,$h);$pdf->MultiCell(40+30,6," Total Pays (R".$_EMPLOYEE_RATE." * ".$tHour_TOTAL."hrs)",1,'L');$pdf->SetFont('arial','B',10);$pdf->SetFillColor(128,246,166);$pdf->SetXY(5+$w+25+25+20+20+20+20+30,$h);$pdf->MultiCell(30,6,"R ".number_format(round($_EMPLOYEE_RATE*$tHour_TOTAL,2),2,"."," "),1,'R',true);$pdf->SetFillColor(255);
		$h+=6;
		$h+=6;$pdf->SetFillColor(255);
		...
		$pdf->SetFont('arial','',10);$pdf->SetTextColor(125);$pdf->Text(130,$h+47.4,$_ORG_NAME." (PTY) Ltd");
			*/
				...
				$pdf->SetTitle($fname." - ".$_GET['_employee']." - ".date("Y-F-d"));
				
				@$pdf->Output(ROOTPATH_1."exports/pdf/campaigns/tmp/".$fname." - ".$_GET['_employee']." - ".date("Y-F-d").".pdf","F");
